...
CREATE INDEX IF NOT EXISTS idx_user_model_access_user_id ON user_model_access(user_id);
...
-- -----------------------------------------------------------------------------
-- 7. user_model_access 表策略
-- -----------------------------------------------------------------------------
DROP POLICY IF EXISTS "user_model_access_select_own" ON user_model_access;
CREATE POLICY "user_model_access_select_own" ON user_model_access FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "user_model_access_insert_own" ON user_model_access;
CREATE POLICY "user_model_access_insert_own" ON user_model_access FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "user_model_access_update_own" ON user_model_access;
CREATE POLICY "user_model_access_update_own" ON user_model_access FOR UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "user_model_access_delete_own" ON user_model_access;
CREATE POLICY "user_model_access_delete_own" ON user_model_access FOR DELETE USING (auth.uid() = user_id);
